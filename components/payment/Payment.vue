<template>
  <div v-touch:swipe="swipeHandler" class="show_tab_navigation">
    <PageHeader
      :close-function="true"
      :title="isTermPaid ? $t('Plaćeno!') : $t('Naplata termina')"
      :background="isTermPaid ? 'green' : ''"
      @back="cancel()"
    ></PageHeader>
    <div v-if="isLoaded && true" class="x-container p-b-150 m-t-10">
        ...
        <div v-if="true" class="activity__box action__buttons">
          <div v-for="(player, index) in filterPlayers" :key="player.id + index" class="action__item__small fw600">
            ...
            <div class="flex__column" @click="$router.push(localePath('/player/' + player.id))">
              ...
              <div v-if="Object.keys(player.memberships).length" class="fw600 is-size-65 has-text-lightblue flex">
                {{ player.memberships[0].name }}
              </div>
              ...
              <template v-if="findPlayersPaymentInfo(player.id).length">
                <div
                  ...
                  :class="findPlayersPaymentInfo(player.id)[0].data.cash ? 'has-text-success' : 'has-text-danger'"
                >
                  <span v-if="findPlayersPaymentInfo(player.id)[0].data.cash">{{ $t('Gotovina:') }}</span
                  ><span v-else>{{ $t('Akontacijski:') }}</span>
            ...
            <div v-if="isTermPaid" class="buttons m-l-auto m-t-5">
              <b-button
                :class="{
                  softshadow: true,
      ...
      <template v-if="!isTermPaid && true">
        <div class="m-t-20">
          <div class="">
            ...
            <div v-if="!isTermPartialyPaid" class="divider-line-1 m-t--15"></div>
              ...
              <div class="activity__box action__buttons">
                ...
                >
                  ...
                  <div class="flex__column">
                    ...
                    <template>
                      <div
                        v-if="Object.keys(player.memberships).length"
                  ...
                  <div v-if="!(!split && isSelected(player))" class="buttons m-l-auto">
                    ...
                    >
                      <span v-if="isSelected(player)">{{ $t('platitelj') }}</span>
                      <span v-else>{{ $t('Odaberi') }}</span>
                    </b-button>
                ...
                <div v-else class="action__buttons w100 p-l-5 p-r-5 m-b--10">
                  ...
                  >
                    ...
                    <div class="m-l-auto has-text-right m-r--15">
                      ...
                      <span>{{
                        (' = ' + (reservation.prices[index] * reservation.prices.length) / filterPlayers.length)
                          | currency
                      }}</span>
                    </div>
                  </div>
    ...
    <modal-payment-stripe v-if="showStripe"></modal-payment-stripe>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deletePayment(id) {
      ...
      const p = new ReservationPayment(this.findPlayersPaymentInfo(id)[0])
      p.for(r)
        .delete()
        .then(() => {
