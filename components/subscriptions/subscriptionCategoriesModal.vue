            ...
            <transition name="fade">
              <div class="activity__box action__buttons">
                <div
                  ...
                  :class="{ selected_payee: selected_subscriptions === index }"
                ...
                >
                  <div class="fw600 align__centar__all">
                    ...
                    <div class="" @click.stop="action(index)">
                      <b-icon
                        :icon="selected_subscriptions === index ? 'user-edit' : 'times-circle'"
                        :type="selected_subscriptions === index ? 'is-white' : 'is-danger'"
                        :size="selected_subscriptions === index ? 'is-medium' : ''"
                        pack="fal"
                        class="m-r-10"
                      >
                      </b-icon>
      ...
      <b-tab-item>
        ...
        <div class="reservation-categories-box">
          ...
          <ValidationProvider name="Tax" rules="required" class="radio__box">
            <div class="block">
              <b-radio v-for="item in taxes" :key="item.id" v-model="newSubscription.tax_class" :native-value="item">
                {{ item.rate }}%
              </b-radio>
            </div>
          </ValidationProvider>
          <template v-if="business_unit.length > 1">
            ...
            <div class="buttons m-t-10 m-b-5">
              <b-button
                ...
                :class="{
                  softshadow: true,
                  'invert-small-button': newSubscription.business_unit === item.name,
                }"
                :icon-right="newSubscription.business_unit === item.name ? 'check' : ''"
                :type="{
                  'is-small': true,
                  noborder: true,
                  'is-primary has-text-white': newSubscription.business_unit === item.name,
                }"
                pack="fal"
                rounded
                @click.prevent="selectBusinessUnit(item.name)"
              >
                {{ item.name }}
              </b-button>
            </div>
...
export default {
  ...
  watch: {
    ...
    club: {
      immediate: true,
      handler() {
        if (this.club.business_data.bank_account && !this.isEditing) {
          this.newSubscription.bank_account = this.club.business_data.bank_account
          this.newSubscriptionPlaceholder.bank_account = this.club.business_data.bank_account
        }
      },
    },
  },
