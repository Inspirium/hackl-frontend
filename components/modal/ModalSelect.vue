<template>
  <div>
    ...
    <div class="filter-list">
      <div class="select-item" @click="send(0)">{{ $t('bezNositelja') }}</div>
      <div v-for="item in options" :key="item" class="select-item" @click="send(item)">
        <div>{{ $t('nositelja', { expr: item === 0 ? 'Bez' : item }) }}</div>
      </div>
    </div>
