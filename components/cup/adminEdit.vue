<template>
  <div v-if="Object.keys(liga).length" class="">
    <div class="label">{{ liga.admins.length === 1 ? $t('Administrator kupa') : $t('Administratori kupa') }}</div>
...
export default {
  ...
  methods: {
    ...
    addAdmin(item) {
      if (_.indexOf(this.admins, item) > -1) {
        this.admins.splice(_.indexOf(this.admins, item), 1)
      } else {
