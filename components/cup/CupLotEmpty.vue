<template>
  <div v-if="true" class="activity__box--p p-l-10 p-r-10 flex-wrap">
      ...
      <div class="flex flex__column align__centar__x">
        ...
        >
          ...
          <span
            ...
            class="is-size-7 x-tag is-white nav__notification has-text-lightblue seed-tag-result-lot-empty radius__full m-r-5 m-l--20"
          ...
          <div v-if="game.is_surrendered === 1" class="add__icon" @click="openModal">
            <div class="x-tag m-l-10 m-r-10 is-red">{{ $t('predaja') }}</div>
          </div>
        </div>
        <div v-else class="align__centar__y">
          <template v-if="!isFirstRound">
            <b-icon icon="clock" type="is-danger" pack="fal" size="is-smaller" class="m-r-10 m-l-5 is-size-6"> </b-icon>
            <h3 class="is-size-6 has-text-danger fw600">{{ $t('ekaProtivnika') }}</h3>
          </template>
          <!--        <template v-else>-->
        ...
        <div v-else class="align__centar__y">
          <template v-if="!isFirstRound">
            <b-icon icon="clock" type="is-danger" pack="fal" size="is-smaller" class="m-r-10 m-t-10 m-l-5 is-size-6">
            </b-icon>
            <h3 class="is-size-6 has-text-danger fw600 m-t-10">{{ $t('ekaProtivnika') }}</h3>
          </template>
          <!--        <template v-else>-->
          <!--          <b-icon icon="check" type="is-primary" pack="fal" size="is-small" class="m-r-10 m-l-5 is-size-6"> </b-icon>-->
          <!--          <h3 class="is-size-7 has-text-black20 fw600">Bez protivnika</h3>-->
    ...
    <div
      ...
      class="add__icon m-t-10 m-b-5 m-l-auto m-r-auto"
      @click="openModal"
    >
      <!--          <b-icon icon="plus-circle" type="is-green" pack="fal" class="m-r-10 cup__add-result"> </b-icon>-->
