    ...
    <div class="activity x-container">
      <div class="activity__results">
        <div class="activity__header m-b-10">
          <nuxt-link :to="localePath('/cup')" class="fw600"
            >{{ $t('turniri') }}
        ...
        <b-table v-else :data="liga" :mobile-cards="false" :bordered="false" class="overflow-hidden softshadow br10">
          <b-table-column
            v-slot="props"
            cell-class="active list__image-col"
            class="active list__image-col"
            field="first_name"
          >
            <nuxt-link
              :to="
              ...
              <div class="list__content">
                ...
                <h3 class="fw600 has-text-lightblue is-size-65">
                  {{
                    $t('key', {
                      expr:
                        props.row.access_level === 'private'
                          ? 'Privatni turnir'
                          : 'Prijavljenih igrača: ' + props.row.players.length,
                    })
                  }}
                </h3>
              </div>
        ...
        <div v-if="!liga.length" class="empty__box">
          <div class="m-b-10 has-text-centered">
            <nuxt-link :to="localePath('/classified')" class="fw600 is-size-4 has-text-black80">{{
              $t('nemaAktivnihTurnira')
            }}</nuxt-link>
          </div>
          <div class="fw600 is-size-54 m-t-10 m-b-15 has-text-centered">{{ $t('pokreniTeniskiTurnir') }}</div>
