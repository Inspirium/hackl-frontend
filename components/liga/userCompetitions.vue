<template>
  <div>
    <!--    <div class="fw500 is-size-55 has-text-lightblue m-t&#45;&#45;20 m-b-20">Odaberi vrstu meča za unos rezultata</div>-->

    <div
      v-show="activeTab === 0"
      class="fw600 is-size-5 has-text-centered has-text-white m-t-30 m-b-20"
      @click="getGames()"
    >
      {{ $t('odaberiVrstuMea') }} <span class="has-text-warning">{{ $t('prijateljskiIliNatjecateljski') }}</span>
      {{ $t('zaUnosRezultata') }}
    </div>
    <b-tabs v-model="activeTab" size="is-medium" position="is-centered" class="hide_tab_navigation">
      <b-tab-item>
        <transition name="fade">
          <div class="is-black-list small-table fw600 flex justify-between m-t-7" @click="$emit('friendly')">
            <div class="flex__column align__centar__y">
              <div class="has-text-white is-size-4">{{ $t('prijateljskiMe') }}</div>
            </div>
      ...
      <b-tab-item>
        ...
        <div class="relative">
          ...
          <div v-if="selectedLeague.classification === 'pyramid'" class="p-b-100 relative">
            ...
            <template v-else>
              <div v-if="!loading" class="rounded-md has-text-centered m-t-40">
      ...
      <b-tab-item>
        ...
        <template v-else>
          <div v-if="!loading2" class="rounded-md has-text-centered m-t-40">
            <b-icon icon="exclamation-square" pack="fal" type="is-white" class="is-large m-t-5"> </b-icon>
            <div class="has-text-centered is-size-5 has-text-white fw600">
              {{ $t('nemateMeevaUNatjecanju', { name: selectedTournaments.name }) }}
            </div>
  ...
  computed: {
    ...
    userInGroup() {
      if (!this.selectedLeague.is_doubles) {
        for (let i = 0; i < this.selectedLeague.groups.length; i++) {
