  ...
  <div v-if="walletTransaction.length" class="member">
    ...
    <div class="activity x-container m-b--20">
      ...
      <div class="buttons m-t-20 m-b-20">
        <b-button
          v-if="club.payment_online && club.payment_accontation"
          class="fw600 softshadow"
          type="is-success"
          pack="fal"
          rounded
          @click="isModalPrepaidStripeActive = true"
        >
          {{ $t('Uplati na račun') }}
        </b-button>
        <b-button
    ...
    <b-modal :active.sync="isModalPrepaidStripeActive" :width="640" scroll="clip">
      <modalPaymentPrepaidStripe
        :title="$t('plaanjeRauna')"
        :subtitle="$t('Unesi podatke o plaćanju računa')"
        :label="$t('iznosRauna')"
        :player="player"
...
import modalPaymentPrepaidStripe from '~/components/modal/ModalPaymentPrepaidStripe'
...
export default {
  ...
  data() {
    return {
      isModalPrepaidStripeActive: false,
