<template>
  <div class="home p-b-200">
    <PageHeader :title="$t('Moji radni nalozi')" :background="'is-primary'" />
    ...
    <div class="relative p-x-16 m-t-20">
      <template>
        ...
        <div v-if="orders.length" class="is-white radius__12 overflow-hidden softshadow">
          ...
          <div v-for="(item, index) in orders" :key="index + 'or'" class="p-r-10 p-l-10 p-t-7 p-b-7 bb1-10">
            <nuxt-link :to="localePath('/admin/workorders/' + item.id)" class="fw600 align__centar__all">
              <div v-if="item.assigner.image" class="list__img">
                <TennisImage :size="[80, 80]" :src="item.assigner.image" class="user_image" alt="" />
              </div>
              <div class="list__content">
                ...
                <h4 class="fw600 is-size-5 m-t--2">{{ item.assigner.display_name }}</h4>
                <h3 v-if="item.note" class="fw600 has-text-lightblue is-size-65">
                  {{ $t('Zadatak') }}:
                  <span class="has-text-black80">{{ item.note }}</span>
                </h3>
              </div>
...
export default {
  ...
  methods: {
    ...
    getOrders() {
      ...
      wo.page(this.offset)
        .where('created_between', this.reservationTimeRange)
        .where('club', this.$store.state.club.id)
        .where('assignee', this.$route.params.id)
