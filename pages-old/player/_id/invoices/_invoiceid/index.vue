  ...
  <div v-if="Object.keys(invoice).length" class="member p-b-500">
    ...
    <div class="flex has-text-white fw600 has-text-centered m-t--2 border-dashed-top">
      ...
      <div :class="returnBckg(invoice)" class="w50 p-all-12 align__centar__all border-dashed-left">
        <h4 v-if="invoice.payment_status === 'PAID'" class="has-text-white fw600 is-size-6 flex align__centar__y">
          {{ $t('Plaćeno') }}
                ...
                <div class="list__content">
                  ...
                  <div class="buttons m-t-50">
                    <b-button
                      ...
                      tag="a"
                      target="_blank"
                      :href="'https://api.tenis.plus/v2/invoice/' + $route.params.invoiceid + '/pdf'"
                    >
...
export default {
  ...
  methods: {
    ...
    getInvoice() {
      this.loading = true
      Invoice.find(this.$route.params.invoiceid).then((res) => {
        this.invoice = res
        this.loading = false
        this.getPayment()
      })
