    ...
    <b-tabs v-model="activeTabPlayers" size="is-medium" class="hide-only-one" position="is-centered">
            ...
            <transition-group name="fade">
              <div v-if="filteredLeftovers.length" key="1" class="activity__header--center m-b-5 m-t-20">
                <div class="fw600 is-size-5">
                  {{
                    $t('nerasporeeni', {
                      expr: liga.is_doubles ? $t('parovi') : $t('igrači'),
                      length: filteredLeftovers.length,
                    })
                  }}
                </div>
              </div>
              <div v-else key="2" class="activity__header--center m-b-5 m-t-20">
                <div class="fw600 is-size-5">{{ $t('Dodaj nove igrače') }}</div>
              </div>
            </transition-group>
            <div class="cup__players-list">
              <div v-for="(player, index) in filteredLeftovers" :key="index">
                <template v-if="liga.is_doubles" class="for-doubles">
                  <div
                    v-if="player.image === 'https://api.tenis.plus/images/user.svg' && player.players.length"
                    class="list__img"
                  >
                    <TennisImage
                      :size="[42, 42]"
          ...
          <div class="p-b-100">
            ...
            >
              <transition-group name="slide">
                ...
                >
                  <div v-if="true" class="activity__header m-b-10 m-t-30">
                    ...
                    >
                      <div v-if="Object.keys(player).length" class="align__centar__y">
                        ...
                        <div class="list__content">
                          ...
                          <h4>{{ $t('bodova', { score: Math.round(player.rating_club) }) }}</h4>
                        </div>
                        <div
                          class=""
                          @click.stop="
                      ...
                      >
                        <div class="align__centar__y">
                          <div class="activity__header p-t-5 p-b-5" @click="insertPlayer(indexGroup)">
                            ...
                            <div class="list__content">
                              <h3 class="has-text-black50 fw600">
                                {{ $t('dodaj2', { expr: liga.is_doubles ? 'par' : 'igrača' }) }}
                              </h3>
                            </div>
                          </div>
                        </div>
                  ...
                  <div v-if="liga.groups.length >= 2" class="buttons">
                    <b-button
                      class="softshadow m-t-20 is-icon-red"
                      type="is-small noborder"
                      pack="fal"
                      icon-right="times-circle"
                      rounded
      ...
      <b-tab-item>
        ...
        <membersSearch
          ...
          @update="deletePlayerLiga($event.id)"
        ></membersSearch>
      </b-tab-item>
    </b-tabs>
...
export default {
  ...
  methods: {
    ...
    getLeague() {
      League.include(['players.players', 'admins', 'groups'])
