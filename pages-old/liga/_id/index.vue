    ...
    <div v-if="Object.keys(liga).length && true">
      ...
      <div v-if="isLigaAdmin.length || isAdmin" class="liga__active__header">
        ...
        <div v-else class="member__info liga__header is-red">
          <div class="fw600 is-size-55 has-text-white p-t-10 p-b-10">
            {{ $t('ligaJeZavrena')
            }}<span v-if="liga.finish_date">: {{ liga.finish_date | moment('DD.MM.YYYY') }}</span>
          </div>
        </div>
        <hero
          :options="heroImage"
      ...
      <b-tabs v-model="activeTabResult" size="is-medium" class="hide-only-one" position="is-centered">
        <b-tab-item>
            ...
            <div class="admin_box">
              <div class="buttons m-t-0">
                ...
                <b-button
                  ...
                  rounded
                  tag="nuxt-link"
                  :to="localePath(`/liga/${$route.params.id}/finish`)"
                >
                  {{ $t('Opcije natjecanja') }}
                </b-button>
              </div>
              <div v-if="playersInterestedList.length" class="buttons m-t-10">
                <b-button
                  class="softshadow m-b-10"
                  type="is-small noborder"
                  pack="fal"
                  rounded
                  @click="activeTabResult = 3"
                >
                  {{ $t('popisZainteresiranihIgraa', { length: playersInterestedList.length }) }}
                </b-button>
              </div>
              <div v-if="liga.finish_date === null" class="">
                <div class="label">{{ $t('unesiRokZaZavretakCiklusa') }}</div>
                <b-field>
                  <b-datepicker
                    v-model="new_finish_date"
          ...
          <template v-if="!userInGroupSingleAndDouble && liga.status === '4'">
            <!--          Single interest-->
            <template v-if="!liga.is_doubles">
              <div v-if="userInGroup === false && !singleInLiga.length" class="x-container m-b-15 m-t-20">
                <div class="fw600 is-size-65 has-text-centered">
                  {{ $t('akoBisteEljeliIgratiUOvojLigiInteresMoetePrijaviti') }}
                </div>
              ...
              <template v-else>
                <div class="x-container m-b-0 m-t-20">
                  <div class="fw600 is-size-5 has-text-centered">{{ $t('iskazaliSteInteresZaOvoNatjecanje') }}</div>
                </div>
                ...
                <div class="buttons m-t-0">
                  <b-button
                    ...
                    icon-right="times"
                    rounded
                    @click="removeInteres(user.team.id)"
                  >
                    {{ $t('odjaviteSe') }}
                  </b-button>
                </div>
            ...
            <template v-else>
              <div
                v-show="liga.status === '4' && !hasShowedIntrestdTeam.length && true"
                class="x-container m-b-15 m-t-20"
              >
                <div class="fw600 is-size-65 has-text-centered">
                  {{ $t('akoBisteEljeliIgratiUOvojLigiParovaInteresMoetePri') }}
                </div>
                <doubles
                  v-show="showDoublesIntrestedBox"
                  :loading-it="loading6"
                  :immidiate="true"
                  class="m-t-20"
                  @listofteams="checkIfShowedInteres($event)"
                  @team="showInterest($event)"
                ></doubles>

                <div v-if="!showDoublesIntrestedBox" class="buttons m-t-0">
                  <b-button
                    class="fw600 m-t-10"
                    type="is-primary"
                    pack="fal"
                    icon-right="check"
                    rounded
                    @click="showDoublesIntrestedBox = true"
                  >
                    {{ $t('prijaviSe') }}
                  </b-button>
                </div>
              </div>
              <template v-if="userInGroup === false && hasShowedIntrestdTeam.length">
                <div class="x-container m-b-0 m-t-20">
                  <div class="fw600 is-size-5 has-text-centered">{{ $t('iskazaliSteInteresZaOvoNatjecanje') }}</div>
                </div>
                <div class="w100 flex align__centar__all m-t-10 m-b-5">
                  <div v-if="hasShowedIntrestdTeam.length" class="flex align__centar__all">
                    <div v-if="true" class="flex">
                      <div
                        v-if="hasShowedIntrestdTeam[0].image === 'https://api.tenis.plus/images/user.svg'"
                        class="flex m-r-10"
                      >
                        <TennisImage
                          :size="[42, 42]"
                          :src="hasShowedIntrestdTeam[0].players[0].image"
                          class="user_image border__white--2"
                          alt=""
                        />
                        <TennisImage
                          :size="[42, 42]"
                          :src="hasShowedIntrestdTeam[0].players[1].image"
                    ...
                    <h3 class="fw600 has-text-black80 is-size-6">{{ hasShowedIntrestdTeam[0].display_name }}</h3>
                  </div>
                </div>
                <div class="buttons m-t-0">
                  <b-button
                    ...
                    icon-right="times"
                    rounded
                    @click="removeInteres(hasShowedIntrestdTeam[0].id)"
                  >
                    {{ $t('odjaviteSe') }}
                  </b-button>
                </div>
          ...
          <!--        <p @click="checkIfShowedInteres()">qweqweqwe</p>-->

          <div v-if="userInGroup !== false && liga.status !== '5' && activeTabResult === 0" class="buttons m-t-30">
            <div class="button__big" @click="activeTabResult = 1">{{ $t('unesiRezultat') }}</div>
          </div>
          <!--        for doubles-->
          <div
            v-if="userInGroupDoubles() !== false && liga.status !== '5' && activeTabResult === 0 && liga.is_doubles"
            class="buttons m-t-30"
          >
            <div class="button__big" @click="activeTabResult = 1">{{ $t('unesiRezultat') }}</div>
          </div>
          <b-tabs v-model="activeTab" size="is-medium" position="is-centered">
            <b-tab-item :label="$t('liga')">
              <b-tabs v-model="activeTabLiga" size="is-medium" position="is-centered" class="hide_tab_navigation">
                <b-tab-item>
                  <div v-if="liga.competition_id" class="buttons m-t-20 m-b-10">
                    <b-button
                      class="softshadow"
                      type="is-small noborder"
                      pack="fal"
                      icon-right="list-ol"
                      rounded
                      @click="activeTabLiga = 1"
                    >
                      {{ $t('Prikaži ukupnu ljestvicu') }}
                    </b-button>
                  </div>

                  <section>
                    <div class="is-size-6 fw600 has-text-centered has-text-black80 m-b-10">
                      {{ $t('ukupnoOdigranoMeeva') }}
                      <span class="has-text-primary"
                        >({{
                          Math.round(((totalMatchesInLiga - totalPlayedMatchesInLiga) / totalMatchesInLiga) * 100)
                        }}%)</span
                      >
                    </div>
                  ...
                  <div
                    v-if="userInGroupSingleAndDouble && liga.groups.length > 2 && userInGroup > 2"
                    class="buttons m-t-15 m-b--10"
                  >
                    <b-button
                      class="softshadow"
                      type="is-small noborder"
                  ...
                  <div class="members p-x-16">
                    <div class="p-b-100">
                      ...
                      >
                        <div class="activity__header m-b-10 m-t-30">
                          ...
                          >
                            {{ group.name
                              ...
                              v-if="isAdmin && isLigaAdmin"
                              icon="cog"
                              size="is-small"
                              pack="fal"
                              type="is-black50"
                              class="cog-for-competiton"
                            >
                            </b-icon>
                          </div>
                          ...
                          <div
                            v-if="showGroupinfo !== indexGroup"
                            class="m-l-auto"
                            @click="openSettings(group, indexGroup)"
                          >
                            <b-button
                          ...
                          <div class="liga__group__settings liga__bckg__red">
                            <section>
                              <div class="is-size-55 fw600 has-text-centered has-text-white m-b-10">
                                {{ $t('odigraniMeevi') }}
                                <span class="is-size-55 fw600 has-text-centered has-text-warning m-b-10">
                                  ({{
                                    Math.round(
                                      ((liga.groups[indexGroup].games.length -
                                        totalUnplayedMatchesInGroup(indexGroup).length) /
                                        liga.groups[indexGroup].games.length) *
                                        100
                                    )
                                  }}%)
                              ...
                              <div v-if="isAdmin && isLigaAdmin" class="liga__group__settings">
                                <section>
                                  <div class="is-size-55 fw600 has-text-centered has-text-white m-b-10">
                                    {{ $t('nazivGrupe') }}
                                  </div>
                                  ...
                                  <b-field class="login__input__container m-b-30">
                                    <b-input v-model="liga.groups[indexGroup].name" pack="fal" rounded> </b-input>
                                  </b-field>
                                  <b-field :label="$t('brojNapredovanja')">
                                    <b-numberinput
                                      v-model="liga.groups[indexGroup].move_up"
                                      :placeholder="$t('nema')"
                                      min="0"
                                      :max="liga.groups[indexGroup].players.length - liga.groups[indexGroup].move_down"
                                      type="is-white"
                                      size="is-small"
                                      controls-rounded
                                      rounded
                                  ...
                                  <b-field :label="$t('brojIspadanja')">
                                    <b-numberinput
                                      v-model="liga.groups[indexGroup].move_down"
                                      :placeholder="$t('nema')"
                                      min="0"
                                      :max="liga.groups[indexGroup].players.length - liga.groups[indexGroup].move_up"
                                      type="is-white"
                                      size="is-small"
                                      controls-rounded
                                      rounded
                                  ...
                                  <!--                            <div class="fw600 is-size-55 m-l-5 m-t-20 m-b-7 has-text-white">-->
                                  <!--                              Različito bodovanje ove grupe-->
                                  <!--                            </div>-->
                                  <!--                            <ValidationProvider name="custom" class="radio__box has-text-black80">-->
                                  <!--                              <div class="block">-->
                                  <!--                                <b-radio v-model="groupCustomPoints" :native-value="true"> Da </b-radio>-->
                                  <!--                                <b-radio v-model="groupCustomPoints" :native-value="false"> Ne </b-radio>-->
                                  <!--                              </div>-->
                                  <!--                            </ValidationProvider>-->
                                  <template v-if="liga.groups_custom_points">
                                    <div class="divider-line-1 m-t-20"></div>
                                    <div class="is-size-4 fw600 has-text-centered has-text-white m-b-10 m-t-10">
                                      {{ $t('posebnoBodovanjeOveGrupe') }}
                                    </div>
                                    <b-field :label="$t('brojBodovaZaPobjedu')">
                                      <b-numberinput
                                        v-model="liga.groups[indexGroup].points"
                                        placeholder="0"
                                        min="0"
                                        type="is-white"
                                        size="is-small"
                                    ...
                                    <b-field :label="$t('brojBodovaZaOsvojeniSet')">
                                      <b-numberinput
                                        v-model="liga.groups[indexGroup].points_loser"
                                        placeholder="0"
                                        min="0"
                                        type="is-white"
                                        size="is-small"
                                    ...
                                    <b-field :label="$t('brojBodovaZaOdigraniMe')">
                                      <b-numberinput
                                        v-model="liga.groups[indexGroup].points_match"
                                        placeholder="0"
                                        min="0"
                                        type="is-white"
                                        size="is-small"
                        ...
                        <!--                   Frozen list-->
                        <div v-if="freezeList(group.players).length" class="">
                          <div class="w100 m-t-15 m-b-7">
                            <div class="fw600 flex align__centar__y">
                              <font-awesome-icon class="h13r" icon="fa-light fa-2xs fa-circle-pause" />{{
                                $t('zaleeniStatus')
                              }}
                            </div>
                          </div>
                          <div class="activity__box action__buttons liga__group__list opacity6">
                            <div v-for="(player, index) in freezeList(group.players)" :key="index">
                              <div v-if="player.pivot.freeze" class="action__item__small-freez p-r-10">
                                <nuxt-link
                                  :to="localePath('/liga/' + $route.params.id + '/' + player.id)"
                                  class="align__centar__y"
                                >
                                  <div class="list__img">
                                    <template v-if="liga.is_doubles" class="for-doubles">
                                      <div
                                        v-if="player.image === 'https://api.tenis.plus/images/user.svg'"
                                        class="flex"
                                      >
                                        <TennisImage
                                          :size="[42, 42]"
                                      ...
                                      <div v-else class="list__img">
                                        <TennisImage
                                          :size="[42, 42]"
                                          :src="player.image"
                                          class="user_image-smaller border__white--2"
                                          alt=""
                                        />
                                      </div>
                                    </template>
                                  ...
                                  <div class="list__content">
                                    <h3 class="fw600 has-text-black80 is-size-65">{{ player.display_name }}</h3>
                                  </div>
                                </nuxt-link>
                                <div class="m-l-auto">
                                  <b-button
                                    v-if="ifPlayerIsFrosen(player)"
                                    class="m-l-auto"
                                    type="is-primary"
                                    rounded
                                    size="is-smaller"
                                    :loading="loadingUnFreeze"
                                    @click="unFreeze(player, indexGroup)"
                                    >{{ $t('odlediStatus') }}</b-button
                                  >
                                </div>
                              </div>
                            </div>
                        ...
                        <div class="buttons m-t-20 m-b--20">
                          <b-button
                            v-if="isLigaAdmin.length > 0 || isAdmin"
                            class="softshadow m-b-10"
                            type="is-small noborder"
                            pack="fal"
                            icon-right="user-edit"
                            rounded
                            @click="
                              selectedGroup = indexGroup
                              activeTabResult = 2
                            "
                          >
                            {{ $t('urediIgrae2') }}
                          </b-button>
                          <template v-if="!freezedSingleAndDoubles && liga.freeze === 1">
                            <b-button
                              v-if="userInGroup === indexGroup || userInGroupDoubles1 === indexGroup"
                              class="softshadow invert-small-button"
                              type="is-small noborder is-primary"
                              pack="fal"
                              :icon-right="!show_details ? 'fa-circle-pause' : 'fa-circle-play' + '' + ''"
                              rounded
                              :loading="loadingFreeze"
                              @click="addFreeze(indexGroup)"
                            >
                              {{ !show_details ? 'Zamrzni status' : 'Odmrzni status' }}
                            </b-button>
                          </template>
                        </div>
                        ...
                        <template v-if="userInGroup === indexGroup">
                          <template v-if="messages.length">
                            <div class="is-size-5 fw600 has-text-centered m-t-20 m-b-5">{{ $t('grupnePoruke') }}</div>
                            <div class="schoolgroup__messages">
                              <template>
                                <div
                                  v-chat-scroll="{ always: false, smooth: true, window: true }"
                                  class="comments p-b-10"
                                >
                                  <div
                                    ...
                                    :class="{
                                      'comments-item': true,
                                      my_comments:
                                        $auth.user.id === comment.player.id && comment.multimedia_type !== 'terms',
                            ...
                            <div class="buttons m-t-20">
                              <b-button
                                tag="nuxt-link"
                                :to="localePath('/messages/' + liga.groups[indexGroup].thread.id)"
                          ...
                          <template v-else>
                            <div class="buttons m-t-20">
                              <b-button
                                ...
                                icon-right="comments"
                                rounded
                                :loading="loadingMessagesLigaGroup"
                        ...
                        <template v-if="userInGroup !== indexGroup && isAdmin">
                          <div class="buttons m-t-20">
                            <b-button
                              class="softshadow m-b-20"
                              type="is-small noborder"
                ...
                <b-tab-item>
                  <div class="buttons m-t-20 m-b-10">
                    <b-button
                      ...
                      rounded
                      @click="activeTabLiga = 0"
                    >
                      {{ $t('nazad') }}
                    </b-button>
                  </div>
                  <div class="fw600 has-text-centered is-size-35">{{ $t('Ukupna ljestvica') }}</div>
                  <div class="p-x-16">
                    <div
                      v-if="Object.keys(ligaCompetition).length"
                      class="fw600 m-b-15 is-size-55 liga-progress__label w100 m-b-10 m-t--10 rounded-md has-text-white flex"
                    >
                      <div v-for="index in ligaCompetition" :key="index" class="liga-progress__separator"></div>
                    </div>

                    <div class="activity__box action__buttons liga__group__list">
                      <div class="liga__legend__status w100">
                        <div class="liga__legend__status_box m-l-auto">
                          <div class="liga__points">{{ $t('bodovi') }}</div>
                        </div>
                      ...
                      <div
                        v-for="(player, index) in ligaCompetition.teams"
                        :key="index"
                        class="action__item__small--liga p-r-10"
                      >
                        <div class="align__centar__y">
                          ...
                          <div class="list__content">
                            <h3 class="fw600 has-text-black80 is-size-6">{{ player.players[0].display_name }}</h3>
                          </div>
                          <div class="liga__points">{{ player.pivot.points }}</div>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <b-tab-item :label="$t('info')" class="p-b-100 x-container">
              ...
              <adminEdit class="m-t--10 m-b-10" :options="liga"></adminEdit>

              <div class="display-data__box m-t-0">
              ...
              <!--          <div class="fw600 is-size-7 has-text-danger">*Ovisno o konačnom broju prijavljenih</div>-->
              <!--        </div>-->
              <div class="activity__header m-b-10 m-t-10">
        ...
        <b-tab-item>
          <div class="x-container">
            ...
            <div class="fw600 is-size-3 m-t-20 m-b-10">{{ $t('odaberiMe') }}</div>
            <div v-if="userInGroup !== false && !liga.is_doubles" class="p-b-100">
              <div v-for="(game, index) in userMatchList" :key="index" class="m-b-10">
                <ligaLotEmpty
                  v-if="game.result === null"
                  :game="game"
                  :isligaadmin="isLigaAdmin.length"
                  ...
                  "
                ></ligaLotEmpty>
                <ligaLotPlayed v-else :game="game" :show-date="true"></ligaLotPlayed>
              </div>
            </div>
            <div v-if="userInGroupDoubles !== false && liga.is_doubles" class="p-b-100">
              <div v-for="(game, index) in userMatchListDoubles" :key="index" class="m-b-10">
                <ligaLotEmpty
                  v-if="game.result === null"
                  :game="game"
                  :is-doubles="liga.is_doubles"
                  ...
                  "
                ></ligaLotEmpty>
                <ligaLotPlayed v-else :game="game" :show-date="true" :is-doubles="liga.is_doubles"></ligaLotPlayed>
              </div>
            </div>
          </div>
        </b-tab-item>
        <!--      Players group editing-->
        <b-tab-item>
          <div class="activity__header--center m-b-5 m-t-15">
            <div class="fw600 is-size-5">
              {{ $t('urediIgrae', { expr: selectedGroup !== null ? liga.groups[selectedGroup].name : '' }) }}
            </div>
          </div>
          <div v-if="true" class="buttons m-t-20 m-b-0">
            ...
            <b-button
              ...
              rounded
              :loading="loading1"
              @click="isModalAddPlayerToGroupActive = true"
            >
              {{ $t('dodajIgraa') }}
            </b-button>
          </div>
          ...
          <membersSearch
            :prepopulated="selectedGroup ? liga.groups[selectedGroup].players : liga.groups[0].players"
            :is-doubles="!!liga.is_doubles"
            @update="deletePlayer($event.id)"
          ></membersSearch>
        </b-tab-item>
        <!--      Players intrested list-->
        <b-tab-item>
          <div class="activity__header--center m-b-5 m-t-15">
            <div class="fw600 is-size-5">{{ $t('zainteresiraniIgrai') }}</div>
          </div>
          <div v-if="true" class="buttons m-t-20 m-b-0">
            <b-button
              class="softshadow m-b-10"
            ...
            <b-button
              ...
              rounded
              :loading="loading2"
              @click="isModalAddInterestedPlayerActive = true"
            >
              {{ $t('dodajIgraa') }}
            </b-button>
          </div>
          ...
          <membersSearch
            :prepopulated="playersInterestedList"
            :show-profile="true"
            :is-doubles="liga.is_doubles"
            @update="removeInteres($event.id)"
          ></membersSearch>
        </b-tab-item>
      </b-tabs>
...
export default {
  ...
  methods: {
    getCompetition() {
      Competition.find(1).then((res) => {
        const x = res
        x.teams.sort((a, b) => b.pivot.points - a.pivot.points)
        this.ligaCompetition = x
      })
    },
    submitMessageInfo() {
