<template>
  <div class="home p-b-150">
    <PageHeader :title="$t('Usluge')" :back="true" :background="'is-primary'" />
          ...
          <div class="has-text-black50 flex flex__column">
            <nuxt-link :to="localePath('/shop/' + item.id)" class="">
              ...
              <div class="shop-meta-content">
                ...
                <div v-if="item.description" class="fw600 is-size-65 has-text-black50 m-t-4">
...
export default {
  ...
  methods: {
    getProducts() {
      ...
      this.loading = true
      const sp = ShopProduct.page(this.offset).where('club', this.$store.state.club.id).where('special', 'service')
      if (!this.isAdmin) {
