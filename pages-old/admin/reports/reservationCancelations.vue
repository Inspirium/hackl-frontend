<template>
  <div class="p-b-150">
    <PageHeader :title="$t('Otkazivanje rezervacije')" />
    <div class="fw600 has-text-centered has-text-black50 w100 is-size-6 m-t-20 m-b-20 p-x-16">
      {{ $t('Popis članova s najviše otkazivanja rezervacija u odabranom vremenskom razdoblju') }}
    </div>
            ...
            >
              ...
              <div class="list__content">
                ...
                <div class="fw600 is-size-65 has-text-black30 align__centar__y flex">
                  ...
                  <!--                  ></b-icon>-->
                  <span class="has-text-black30">{{ $t('Ukupno rezervacija') }}:</span
                  ><span class="m-l-4 has-text-black50">{{ item.reservations_count }} </span>
                </div>
                <div class="fw600 is-size-65 has-text-black30 align__centar__y flex">
                  <span class="has-text-black30">{{ $t('Postotak otkazivanja') }}:</span
                  ><span
                    class="m-l-4"
                    :class="
                      Math.round((item.inactive_reservations_count / item.reservations_count) * 100) > 25
                        ? 'has-text-danger'
                        : 'has-text-black50'
                    "
                    >{{ Math.round((item.inactive_reservations_count / item.reservations_count) * 100) }}%
                  </span>
                </div>
              </div>
              <div class="">
                <div class="fw600 has-text-black80 is-size-5 m-l-auto nowrap">
                  {{ item.inactive_reservations_count }}
                </div>
              </div>
            </nuxt-link>
          </div>
